<template>
  <view class="upFile">
    <view class="media" v-if="!fileType">
      <view v-if="!type" class="multiple">
        <view class="imageList">
          ...
          >
            ...
            <view
              class="close"
              @click="removeFile('multiple', index)"
              v-if="compile"
            >
      ...
      <view v-else class="single">
        ...
        >
          <image
            mode="aspectFill"
            :src="(fileList[0] && fileList[0][valueKey]) || background"
            @click="uploadFile('single')"
          ></image>
          <view
            class="close"
            @click="removeFile('single')"
            v-if="fileList.length && compile"
          >
            <image :src="closeImg"></image>
          </view>
        </view>
    ...
    <view class="accessory" v-else> </view>
  </view>
</template>
<script>
export default {
  ...
  props: {
    ...
    background: {
      type: String,
      default:
        "https://vkceyugu.cdn.bspapp.com/VKCEYUGU-aliyun-hglbk3dir3rbd07f5a/2efd0ea0-5718-11eb-8a36-ebb87efcf8c0.png",
    },
    // 删除图片
    closeImg: {
      type: String,
      default:
        "https://vkceyugu.cdn.bspapp.com/VKCEYUGU-aliyun-14mxvwe8lug756a025/12e55e20-50b0-11eb-b680-7980c8a877b8.png",
    },
                ...
                let upFile = async (data) => {
                  if (data.length > count) {
                    ...
                    const result = await uniCloud.uploadFile({
                      ...
                      cloudPath:
                        data[count].name ||
                        data[count].path.match(/[^\/]+$/)[0], // 只有h5能获取到图片的名字
